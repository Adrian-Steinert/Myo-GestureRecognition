...
## origin of https://gist.github.com/julienr/6b9b9a03bd8224db7b4f modified for python 3
def parse_classification_report(clfreport):
    ...
    # Remove empty lines
    line_length = len(lines) - 2  # 2 empty lines removed
    lines = filter(lambda l: not len(l.strip()) == 0, lines)

    # Starts with a header, then score for each class and finally an average
    header = next(lines)
    cls_lines = [next(lines) for i in range(1, line_length - 2)]  # stop at second last item
    avg_line = next(lines)
    ...
    # average
    data['$\mathlarger{\mathlarger{\\varnothing}}$|gesamt'] = parse_line(avg_line)[1:]
...
def report_to_latex_table(data):
    ...
    out += "Klasse & Precision & Recall & F-Measure & Support\\\\\n"
    out += "\hline \hline\n"
